...
type error =
  (* | TypeError *)
  (* | IndexOutOfBounds *)
  | DimensionMismatch (* | TypeCastError *)
...
let check_subtype loc p soustypede trucquiextends =
  ...
  | a, b -> if a <> b then PrimitiveSubtype |> tpraise else true
...
let find_class_def p class_name =
  try List.find (fun cls -> cls.class_name = class_name) p.classes
...
let find_interface_def p interface_name =
  try List.find (fun i -> i.interface_name = interface_name) p.interfaces
...
let get_class_methods p class_def =
  let definterfaces =
    List.fold_left
      (fun acc name -> find_interface_def p name :: acc)
      [] class_def.implemented_interfaces
  in
  let machin =
    List.flatten (List.map (fun inter -> inter.methods) definterfaces)
  in
...
let rec find_method_def p defclass meth_name =
  ...
  | None -> (
      ...
      | None ->
          MethodNotFound meth_name |> tpraise
...
let typecheck_prog (p : program) : program =
  ...
  let type_error ty_actual ty_expected =
    tpraise (UnexpectedType (ty_expected, ty_actual))
  in
  let check_eq_type loc expected actual =
    if expected <> actual then type_error actual expected
  in

  (* let ( =?= ) expect got = expect = got in *)
  let rec check_expr (e : expr) env_stack : expr =
    ...
    let check_eq_type exp got = check_eq_type e.loc exp got in
    ...
    | Unop (u, e) -> (
        ...
        | TypeCast newType -> (
            if is_primitive newType then (
              print_endline "boloss";
              localized_tpraise PrimitiveSubtype e.loc);
            try
              newType <:? typed_e.annot;
              (* check_subtype p newType typed_e.annot ;  *)
              derive_expr newType typed_e.expr
            with _ -> failwith "wtf todo"
    ...
    | New (class_name, generics) ->
        ignore (find_class_def p class_name);
        (*checks existance*)
  ...
  and type_mem_access m stack_env loc : typ =
    let rec reduce_dim t = function
      | [] -> t
      | hd :: tl -> (
          check_eq_type loc TInt (check_expr hd stack_env).annot;
          match t with
    ...
    | Field (obj, field_name) ->
        ...
        let rec find_familly c courrant =
          try
            let _, res, visible =
              List.find (fun (k, _, _) -> k = field_name) c.attributes
            in
            (if eou <> This then
               match visible with
               | Public -> ()
  ...
  and check_instr i ret stack_env : instr =
    ...
    let check_eq_type exp got = check_eq_type i.loc exp got in
    let ( <: ) a b = check_subtype i.loc p b a in
    let ( <:? ) a b = if not (a <: b) then SubTypeError (a, b) |> tpraise in
    ...
    | Set (m, e) ->
        ...
        if is_primitive expect_type |> not then expect_type <:? typed_e.annot
        else check_eq_type expect_type typed_e.annot;
  ...
  let typed_classes =
    let class_match_interface (class_def : class_def)
        (interface_def : interface_def) : unit =
      ...
      in
      List.iter f
        meth_with_body (*checks implementation for methods without default*)
    in
